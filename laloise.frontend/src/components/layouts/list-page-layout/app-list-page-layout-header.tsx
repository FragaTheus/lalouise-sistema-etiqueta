...
export default function AppListPageLayoutHeader() {
  ...
  const [role, setRole] = useQueryState("role", { shallow: false });

  return (
    <div className="fixed w-full max-h-min flex flex-col border-b bg-card z-10">
      ...
      <div className="py-2 px-4 overflow-x-auto inline-flex gap-2 no-scrollbar ">
        {filters.map((filter) => (
          <Toggle
            key={filter.value}
            pressed={role === filter.value}
            onPressedChange={(isOn) => setRole(isOn ? filter.value : null)}
