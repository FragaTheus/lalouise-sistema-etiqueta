...
export default function AppListPageLayoutHeader() {
  return (
    <div className="fixed w-full max-h-min mt-17 lg:mt-0 flex flex-col">
      <div className="bg-card flex py-2 px-4 border-b">
      ...
      <div className="py-2 px-4 overflow-x-scroll flex gap-2 no-scrollbar">
        {filters.map((filter, i) => (
          <Toggle
            key={filter}
            className="rounded-full bg-transparent border border-gray-300 text-muted-foreground cursor-pointer hover:bg-primary/5 data-[state=on]:bg-primary/20 data-[state=on]:text-primary data-[state=on]:border-primary p-2 inline-flex group/filter min-w-30"
          >
            <Filter />
            <span>Filtro</span>
