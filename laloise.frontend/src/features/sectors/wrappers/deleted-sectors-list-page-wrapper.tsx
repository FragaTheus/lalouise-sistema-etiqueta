...
import { useDeletedSectors } from "@/features/sectors/hooks/use-deleted-sectors";
...
function DeletedSectorsClient() {
  const { offset, pageSize } = useListPagination();
  const { search } = useListFilters({});

  const { data, isLoading, error, refetch } = useDeletedSectors({
  ...
  return (
    <ListPageLayout
      ...
      actionHref="/painel/setores/deletados"
      placeholder="Busque por nome"
      data={sectors}
      columns={COLUMNS}
      totalItems={totalItems}
      caption="Lista de setores deletados"
    />
  );
}

export default function DeletedSectorsListPageWrapper() {
  return (
    <SectorsSuspenseWrapper>
      <DeletedSectorsClient />
    </SectorsSuspenseWrapper>
  );
}
