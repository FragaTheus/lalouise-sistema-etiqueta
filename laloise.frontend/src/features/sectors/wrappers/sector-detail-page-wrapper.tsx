...
import { updateSectorSchema } from "@/features/sectors/constants/schemas/update-sector-schema";
...
export default function SectorDetailPageWrapper() {
  ...
  const updateSectorMutation = useUpdateSector(sectorId);
  const deleteSectorMutation = useDeleteSector(sectorId);
  ...
  const sectorItemConfig: AppItemInfoProps = {
    ...
    subtitle: `ID: ${data.id}`,
    isProfile: false,
    updateMutation: updateSectorMutation,
    deleteMutation: deleteSectorMutation,
    editConfig: {
      title: "Editar Setor",
      description:
        "Atualize os dados do setor. Deixe os campos em branco para não alterá-los.",
      schema: updateSectorSchema,
      fields: updateSectorFields,
      defaultValues: {
        name: data.name,
